...
#ifndef     LOG_USING_INFO
/* 编译期信息日志开关：生产环境设为 0 可完全剔除 logInfo() 代码 */
#define     LOG_USING_INFO     1
#endif
#define     LOG_BUFFER_SIZE    128              /**< log输出缓冲大小 */
...
#define logFormat(text, level, fmt, ...)                                   \
        do {                                                               \
            if (LOG_ENABLE) {                                              \
                logWrite(LOG_ALL_OBJ, level,                               \
                         text " [%s:%d] " fmt LOG_END,                    \
                         LOG_TIME_STAMP, LOG_TAG, __func__, __LINE__,       \
                         ##__VA_ARGS__);                                    \
            }                                                              \
        } while (0)
...
#else
/* 信息日志关闭时不展开参数，避免生产版本保留格式化和副作用代码 */
#define logInfo(...) \
        do { } while (0)
#endif
