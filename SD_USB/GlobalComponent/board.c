...
platform_uart_client  uart1_c;

static int shell_board_init(void)
{
	platform_uart_device *uart1_bus;
	
	/*
	 * 调之前 main 已经调过 platform_init(),  ops 注册已经做完, 这里只管拿 bus 句柄.
	 */
	uart1_bus = platform_uart_get_device(UART_1);
	
	if (!uart1_bus) 
	{
		return -1;
	}
	uart1_c.bus=uart1_bus;
	return 0;
}
