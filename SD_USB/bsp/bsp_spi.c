...
static uint8_t spi1_rx_buffer[SPI1_DMA_FRAME_SIZE];
static uint8_t spi1_tx_buffer[SPI1_DMA_FRAME_SIZE];

SPI1_DMA SPI1_RxTx;

static volatile uint8_t spi1_exchange_started = 0U;
...
{
  uint16_t arm_len = SPI1_DMA_BLOCK_SIZE;
...
static HAL_StatusTypeDef SPI1_TryArmIfIdle(void)
{
  if (spi1_exchange_started == 0U)
  {
    return HAL_BUSY;
  }
...
void bsp_SPI1_NssExtiCallback(void)
{
  ...
  if (SPI1_ReadNssLevel() != GPIO_PIN_RESET)
  {
	  ...
	  if (rx_cnt<=SPI1_DMA_BLOCK_SIZE-20)
      {
		  SPI1_RxTx.rx_reality_cnt = SPI1_DMA_BLOCK_SIZE-20;
	  }
	  else if(rx_cnt>(SPI1_DMA_BLOCK_SIZE-20)&&rx_cnt<=SPI1_DMA_BLOCK_SIZE)
	  {
		  SPI1_RxTx.rx_reality_cnt = SPI1_DMA_BLOCK_SIZE;
	  }
      if (SPI1_RxTx.rx_reality_cnt > 0U)
      {
	    memcpy(SPI1_RxTx.rx_shadow, spi1_rx_buffer, SPI1_RxTx.rx_reality_cnt);
	    SPI1_RxTx.rx_ping_ready = 1U;
	    SPI1_RxTx.tx_reality_len = SPI1_RxTx.rx_reality_cnt;
      }
