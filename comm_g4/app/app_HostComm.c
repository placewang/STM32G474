...
/* ----- 填充发送帧的固定字段并计算校验和 ----- */
static void hostCommReturnFill(uint8_t dev_type)
{
    int      check_len=0;
...
static inline void HostCommDataCopy(uint8_t * cdata,const uint8_t* pdata,const int len)
{
    switch (len)
    {
        case 8: cdata[7] = pdata[7];__attribute__((fallthrough)); /* fall-through */
        case 7: cdata[6] = pdata[6];__attribute__((fallthrough)); /* fall-through */
        case 6: cdata[5] = pdata[5];__attribute__((fallthrough)); /* fall-through */
        case 5: cdata[4] = pdata[4];__attribute__((fallthrough)); /* fall-through */
        case 4: cdata[3] = pdata[3];__attribute__((fallthrough)); /* fall-through */
        case 3: cdata[2] = pdata[2];__attribute__((fallthrough)); /* fall-through */
        case 2: cdata[1] = pdata[1];__attribute__((fallthrough)); /* fall-through */
        case 1: cdata[0] = pdata[0];__attribute__((fallthrough)); /* fall-through */
...
/*足式CAN数据填充*/
static void FootCanDataFilling(FootFrameStruct *Foot_type,QUEUE_DATA_T *Candata, int cNum)
{
	if(0==cNum)
	{
...
/*轮式CAN数据填充*/
static void WheelCanDataFilling(wheelFrameStruct *Wheel_type,QUEUE_DATA_T *Candata, int cNum)
{
	if(0==cNum)
	{
		switch (Candata->StdId)
...
return 0
*/
static int MotorDataFilling(uint8_t F_type,QUEUE_DATA_T *Candata, int cNum)
{
	uint32_t clen=Candata->DLC;

	/*如果电机回数据不足8B填充0*/
	if(Candata->DLC<8)
	{
		for(uint32_t c=clen;c<DATA_BLOCK_SIZE;c++)
		{
			Candata->Data[c]=0;
		}
	}/*兼容海泰电机*/
...
static void SendRxCanData(uint8_t S_type)
{
