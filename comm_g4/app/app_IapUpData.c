 ...
 */
static uint32_t crc32_zlib(const unsigned char *data, size_t len)
{
...
static inline void EraseSectors_flashBlock(uint32_t flag)
{
	if(0!=flag)
	{
		...
		bsp_FlashEraseSectors(FLASH_RUNING_AREA_BACKUP,23);
	}
}
/*
回包填充
...
升级成功写参数标记到存储区地址
*/
static void Update_writeflag(uint32_t FLAG_addr,uint32_t *dataF,uint32_t DataLen,uint16_t indx)
{
	uint32_t  writeLen=16;
...
升级请求数据包处理
*/
static void UpdateRequestDataPack(UpDataPack *rec,UpDataPack* trm)
{
	...
	/*存参数信息写入标志区*/
	writeflash_data[0]=CMD_UPGRADE_START; 
...
升级数据包写入flash
*/
static void UpdateDataPackWriteToFlash(UpDataPack *rxf,UpDataPack* txf)
{
...
升级数据最后一包写入flash
*/
static void LastBatchData(UpDataPack *rxl,UpDataPack* txl)
{
	uint32_t packlen=128;
